...
$db = mysqli_connect(HOST_NAME, USER_NAME, USER_PASS, DB_NAME);
if(!$db){die("Can not connect: " . mysql_connect_error()); }
...
function show_form($errors = ''){
	...
	// フォームがサブミットされたら、サブミットされたパラメータからデフォルトを取り出す
	if(isset($_POST['_submit_check'])){
		if(!isset($_POST['is_spicy'])){ // 辛さのチェックがついてなければ
			$_POST['is_spicy'] = 'no';
		}
		$defaults = $_POST;
	}else{
	...
	if(is_array($errors)){
		$error_text = '<tr><td>右記のエラーを修正して下さい :';
...
<tr><td>辛さ:</td>
<td><?php input_select('is_spicy', $defaults, $spicy_choice); ?>
はい</td></tr>

<tr><td colspan="2" align="center"><?php input_submit('search', '料理検索'); ?>
</td></tr>
...
function process_form(){
	...
	if(mb_strlen(trim($_POST['dish_name']))){
		...
		// mysql_real_escape_string()は、値のまわりに単一引用符を
		// 追加しないので、クエリの$dishを単一引用符で囲む必要がある
